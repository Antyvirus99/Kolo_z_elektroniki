...
#define SELECT()	PORTD &=~ (1<<PD4)	/* CS = L */
#define	DESELECT()	PORTD |=  (1<<PD4)	/* CS = H */
#define	MMC_SEL		!(PORTD &  (1<<PD4))	/* CS status (true:CS == L) */
...
)
{
	...
	if (send_cmd(CMD17, lba) == 0) {		/* READ_SINGLE_BLOCK */
		...
		bc = 40000;
		do {							/* Czekaj na pakied danych */
			rc = rcv_spi();
		} while (rc == 0xFF && --bc);

		if (rc == 0xFE) {				/* Pakiet danych dotar³ */
			bc = 514 - ofs - cnt;

			/* Omiñ wczytywanie bajtów */
			if (ofs) {
				do rcv_spi(); while (--ofs);
			}

			/* Odbierz czêœæ sektora */
			if (buff) 
			//if (1)
			{	/* Przechowywanie danych w pamiêci */
				do {
					*buff++ = rcv_spi();
				} while (--cnt);
			} else {	/* Przekazuje dane do zewnetrznego strumienia*/
				do {
//					FORWARD(rcv_spi());
				} while (--cnt);
			}
